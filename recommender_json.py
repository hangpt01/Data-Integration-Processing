...
def parse_args():
    ...
    parser.add_argument('--save_dir', type=str, default='hdfs://localhost:9000/user/Asus/model_pipeline_hdfs_movie', help='Directory to save model and pipeline')
...
class ALSModelPipeline():
    ...
    def train(self, df):
        ...
        if self.hdfs_exists(pipeline_model_path) and self.hdfs_exists(model_path):
            ...
            self.init_model()
            # transformed_data.filter(filtered_df['rating'].isNull()).show()
            row_count = transformed_data.count()
...
if __name__ == "__main__":
    ...
    # HDFS file paths
    file_path = "hdfs://localhost:9000/user/Asus/data.json"
    ...
    # Load the JSON file as an array of records
    df = spark.read \
        .option("multiline", "true") \
        .schema(schema) \
        .json(file_path)

    # Get the count of reviews per movie
    movie_counts = df.groupBy("movie").count()

    # Filter for movies with at least 100 reviews
    movies_with_100_reviews = movie_counts.filter(movie_counts['count'] >= 10).select("movie")

    # Join to keep only rows with these movies in the original DataFrame
    filtered_df = df.join(movies_with_100_reviews, "movie", "inner")
